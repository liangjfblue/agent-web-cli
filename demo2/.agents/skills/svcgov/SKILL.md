...
All commands run from the `demo2/` directory:
...
node cli.js logs                           # all logs
node cli.js logs --service gateway         # gateway logs only
node cli.js logs --level error             # errors only across all services
node cli.js logs --service gateway --level error  # gateway errors only

# Database
node cli.js tables                         # list tables
node cli.js table users                    # preview users table data
node cli.js table orders                   # preview orders table data
node cli.js table products                 # preview products table data
...
- **"服务状态怎么样"** → `node cli.js services` → summarize healthy/degraded/down
- **"gateway有没有报错"** → `node cli.js logs --service gateway --level error`
- **"order-service怎么了"** → `node cli.js logs --service order-service` → look at warn/error
- **"数据库有哪些表"** → `node cli.js tables`
- **"看看users表的数据"** → `node cli.js table users`
